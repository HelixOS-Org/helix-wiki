...
const TOOL_LINKS = [
  ...
  { href: "/blog", icon: "📝", labelKey: "tool.blog", descKey: "tool.blog_desc", hoverColor: "group-hover/item:text-amber-300" },
  { href: "/contributors", icon: "👥", labelKey: "tool.contributors", descKey: "tool.contributors_desc", hoverColor: "group-hover/item:text-pink-300" },
] as const;
...
export default function Navbar() {
  ...
  /* ── Scroll detection for navbar shrink + link fade ── */
  useEffect(() => {
    let ticking = false;
    function onScroll() {
      if (!ticking) {
  ...
  /* ── Fade-out class for links that disappear on search focus ONLY ── */
  const shouldFade = searchFocused;
  ...
  return (
    <header role="banner" className="fixed top-0 w-full z-50 overflow-visible">
      ...
      <nav aria-label="Primary navigation"
        className={`border-b border-white/[0.06] transition-all duration-500 ease-out overflow-visible
          ${scrolled
            ? "bg-black/85 backdrop-blur-2xl shadow-[0_4px_30px_rgba(123,104,238,0.08)]"
            : "bg-black/70 backdrop-blur-xl supports-[backdrop-filter]:bg-black/50"
          }`}>
        <div className={`max-w-7xl mx-auto px-6 flex items-center justify-between gap-4 transition-all duration-500 ease-out overflow-visible ${scrolled ? "h-[3.5rem]" : "h-[4.5rem]"}`}>
          ...
          <Link href="/" aria-label="Helix OS — Home"
            ...
            <div className={`transition-all duration-500 ease-out group-hover:scale-105 ${scrolled ? "w-6 h-6" : "w-8 h-8"}`} aria-hidden="true"><HelixLogo /></div>
            <span className={`font-bold tracking-widest select-none flex items-center gap-2 transition-all duration-500 ease-out ${scrolled ? "text-lg" : "text-xl"}`}>
              HELIX
              <span className="px-2 py-0.5 text-[10px] font-semibold tracking-[0.2em] rounded-md bg-gradient-to-r from-helix-blue/20 via-helix-purple/20 to-helix-accent/20 border border-helix-purple/40 text-zinc-300 leading-none">OS</span>
            </span>
          </Link>

          {/* ── Search — always visible, grows on scroll, EXPANDS on focus ── */}
          <div className={`hidden lg:block ml-2 mr-auto overflow-visible z-40 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]
            ${searchFocused
              ? "flex-1 min-w-0"
              : scrolled
                ? "w-80"
          ...
          `} role="search" aria-label="Search documentation">
          ...
          <div className="hidden lg:flex items-center gap-1.5 text-[15px] shrink-0" role="menubar" aria-label="Main menu">
            ...
            {/* Docs — fades out on scroll */}
            <div className={`transition-all duration-500 ease-out ${fadeClass}`}>
              <Link href="/docs" role="menuitem"
                className={`px-4 py-2.5 rounded-xl font-semibold transition-all duration-200 flex items-center gap-2 whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-helix-purple
                  ${isDocsPage
            ...
            {/* Tools dropdown — fades out on scroll */}
            <div className={`transition-all duration-500 ease-out ${fadeClass}`}>
              <div ref={toolsRef} className="relative" onKeyDown={handleToolsKeyDown}>
                <button ref={toolsBtnRef} onClick={() => setToolsOpen(o => !o)}
                  ...
                  className="px-4 py-2.5 rounded-xl text-zinc-400 hover:text-white hover:bg-white/[0.06] transition-all flex items-center gap-2 cursor-pointer whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-helix-purple">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" aria-hidden="true">
            ...
            <div className={`transition-all duration-500 ease-out ${fadeClass}`}>
              <a href="https://github.com/HelixOS-Org/helix" target="_blank" rel="noopener noreferrer"
                ...
                className="px-4 py-2.5 rounded-xl text-zinc-400 hover:text-white hover:bg-white/[0.06] transition-all flex items-center gap-2 whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-helix-purple">
            ...
            {/* ── Separator dot (visible when scrolled or search focused) ── */}
            <div className={`transition-all duration-500 ease-out ${shouldFade ? "opacity-100 w-1" : "opacity-0 w-0"}`}>
              <div className="w-1 h-1 rounded-full bg-gradient-to-br from-helix-purple to-helix-accent" />
            </div>

            {/* Download — ALWAYS VISIBLE */}
            <Link href="/download" aria-label="Download Helix OS ISO"
              className={`ml-1 px-5 py-2 rounded-full border text-sm font-bold transition-all duration-500 flex items-center gap-2 whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-helix-blue focus-visible:ring-offset-2 focus-visible:ring-offset-black
                ${pathname === "/download"
                ...
                ${shouldFade ? "shadow-[0_0_20px_rgba(74,144,226,0.15)]" : ""}
              `}>
            ...
            <Link href="/donate" aria-label="Support Helix OS — Donate"
              className={`ml-1 px-5 py-2 rounded-full border border-pink-500/30 bg-pink-500/10 text-pink-400 text-sm font-bold hover:bg-pink-500/20 hover:border-pink-500/50 hover:text-pink-300 transition-all duration-500 flex items-center gap-2 whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pink-500 focus-visible:ring-offset-2 focus-visible:ring-offset-black
                ${shouldFade ? "shadow-[0_0_20px_rgba(236,72,153,0.12)]" : ""}
              `}>
